...
func getFoodsAndSymptoms(service *Service, fromDate time.Time, toDate time.Time, symptomIds []uint) ([]Result, error) {
	...
	var err error = service.db.Debug().
		Select(
			...
			"SUM(CASE WHEN condition_events.date BETWEEN meals.date  AND meals.date + interval '72 hour' THEN 1 ELSE 0 END) as hours72",
			"SUM(CASE WHEN condition_events.date BETWEEN meals.date  AND meals.date + interval '24 hour' THEN 1 ELSE 0 END) as hours24",
			"SUM(CASE WHEN condition_events.date BETWEEN meals.date  AND meals.date + interval '1 hour' THEN 1 ELSE 0 END) as hours1",
		).
		Table("meals").
