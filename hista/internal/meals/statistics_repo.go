...
func (r *MealRepository) AggregateNutrition(ctx context.Context, truncateUnit string, from *time.Time, to *time.Time) (NutritionStatistics, error) {
	var nutrition NutritionStatistics
	...
	tx := r.db.Model(&Meal{}).Session(&gorm.Session{SkipDefaultTransaction: true}).
